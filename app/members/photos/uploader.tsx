...
export default function PhotoUploader({disabled,onBusy,onComplete}:{disabled:boolean,onBusy:(v:boolean)=>void,onComplete:()=>Promise<void>}){
 ...
 async function upload(){if(lock.current)return;lock.current=true;setRunning(true);onBusy(true);setNotice('');let successes=0,failures=0;
  try{for(const item of items.filter(x=>x.status!=='done')){
   ...
   const text=(item.caption||caption).trim();
 ...
 return <form className="panel" style={{marginTop:24}} onSubmit={e=>{e.preventDefault();upload()}}>
  ...
  <label className="field"><span>Caption for this batch (optional)</span><input maxLength={180} value={caption} onChange={e=>setCaption(e.target.value)} disabled={disabled||running} placeholder="An evening of pétanque with the club"/><small>Leave blank to upload without captions, or add individual captions below.</small></label>
  {items.length>0&&<div className="upload-list">{items.map(item=><div className="upload-item" key={item.id}><div className="upload-name"><b>{item.file.name}</b><span>{(item.file.size/1024/1024).toFixed(1)} MB · {item.status==='done'?'Uploaded':item.status==='uploading'?'Uploading…':item.status==='error'?'Needs attention':'Ready'}</span></div><label className="field"><span className="subtle">Individual caption (optional)</span><input maxLength={180} value={item.caption} disabled={disabled||running||item.status==='done'} onChange={e=>update(item.id,{caption:e.target.value})} placeholder={caption||'No caption needed'}/></label>{item.error&&<p className="error">{item.error}</p>}</div>)}</div>}
