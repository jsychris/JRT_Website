...
export async function POST(request:Request){
 ...
 if(request.headers.get('content-type')?.startsWith('application/json')){
  const p=await request.json().catch(()=>null);if(!p||typeof p.id!=='string')return reply({error:'Invalid request.'},400);
  ...
  if(p.action==='delete'){
   ...
   d.prepare('DELETE FROM photos WHERE id=?').run(p.id);
  ...
  d.prepare('UPDATE photos SET status=?,reviewed_by=?,reviewed_at=? WHERE id=?').run(p.action==='approve'?'approved':'rejected',m.id,new Date().toISOString(),p.id);
