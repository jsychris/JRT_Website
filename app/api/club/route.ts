...
export async function POST(request:Request){try{const origin=request.headers.get('origin');if(!origin||origin!==(process.env.APP_URL?new URL(process.env.APP_URL).origin:new URL(request.url).origin))return fail('Please use the form on this site.',403);const u=await identity();if(!u)return fail('Please sign in.',401);const raw=await request.text();if(raw.length>100000)return fail('This entry is too long.');let p:any;try{p=JSON.parse(raw);}catch{return fail('Invalid request.');}if(!p||typeof p!=='object'||Array.isArray(p))return fail('Invalid request.');const m=await member();const now=new Date().toISOString();const name=String(p.name||'').trim().slice(0,100);
