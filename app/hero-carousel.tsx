...
export default function HeroCarousel({photos}:{photos:{src:string,caption:string}[]}){
 ...
 return <div className="hero-image hero-carousel" role="region" aria-roledescription="carousel" aria-label="Life at Jersey Round Table">
  <div className="hero-viewport" ref={ref}><div className="hero-slides">{photos.map((p,i)=><div className="hero-slide" role="group" aria-roledescription="slide" aria-label={`${i+1} of ${photos.length}`} key={p.src}><img src={p.src} alt={p.caption||'Jersey Round Table club photo'} loading={i===0?'eager':'lazy'} fetchPriority={i===0?'high':'auto'}/></div>)}</div></div>
